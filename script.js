        ...
        : list.map(p => `
            <div class="product-card" data-cat="${p.category}" data-id="${p.id}">
              <div class="product-img-wrap" style="background: linear-gradient(135deg, ${p.color}, #f5f5f3);">
                ...
                <div class="product-overlay">
                  <div class="overlay-actions">
                    ...
                    <button class="overlay-btn ghost" onclick="viewProduct(${p.id})">Details</button>
                  </div>
                </div>
              </div>
              <div class="product-body">
                ...
                <div class="product-specs">${p.specs.map(s => `<div class="spec"><svg viewBox="0 0 24 24"><path d="M12 1L3 5v6c0 5.55 3.84 10.74 9 12 5.16-1.26 9-6.45 9-12V5l-9-4z"/></svg>${s}</div>`).join("")}</div>
    ...
    function applyFilters() {
      ...
      if (currentSearch) list = list.filter(p => p.name.toLowerCase().includes(currentSearch) || p.desc.toLowerCase().includes(currentSearch));
      renderProducts(list);
    }
    ...
    function addToCart(id) {
      ...
      toast.style.cssText = `position:fixed;bottom:80px;right:32px;z-index:5000;background:var(--fire);color:white;padding:14px 20px;font-family:var(--font-body);font-size:13px;font-weight:700;letter-spacing:1px;animation:toastIn 0.4s ease;clip-path:polygon(6px 0%,100% 0%,calc(100% - 6px) 100%,0% 100%);box-shadow:0 8px 24px rgba(255,106,0,0.4);`;
    ...
    function viewProduct(id) {
      const p = products.find(x => x.id === id);
      alert(`Product: ${p.name}\nPrice: ₹${p.price}/unit\n\nFor full product details, please contact our sales team:\n📞 +91 98765 43210\n✉ info@ignis.in`);
    }
    ...
    document.addEventListener("mousemove", e => {
      ...
      cursor.style.left = tx - 6 + "px"; cursor.style.top = ty - 6 + "px";
    });
    setInterval(() => {
      cx += (tx - cx) * 0.15; cy += (ty - cy) * 0.15;
      trail.style.left = cx - 18 + "px"; trail.style.top = cy - 18 + "px";
    }, 16);
